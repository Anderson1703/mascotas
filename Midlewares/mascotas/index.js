...
midle.saveMascotas = (req, res, next) => {
  ...
  if (data.especie && data.raza && data.nombre, data.userId) {
    ...
    userModel.findById({_id:data.userId})
      .then(respon => {
        mascotasModel
          .create(newMascota)
          .then((result) => {
            const idMascota = result._id;
            const mascotasUpdated = [...respon.mascotas, idMascota]
            userModel.findByIdAndUpdate({ _id: data.userId }, { mascotas: mascotasUpdated })
              .then(() => {
